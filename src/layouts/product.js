                ...
                <main className="content">
                    <section className="product content__row content__row--full-width">
                        ...
                        <figure className="product__figure">
                            <Picture {...this.props} image={_.get(this.props, 'page.default_original_image', null)} alt={_.get(this.props, 'page.title', null)} cssClass={'product__image'} />
                        </figure>
                        <div className="product__details">
                            ...
                            {_.get(this.props, 'page.category', null) && ((() => {
                                let category_page = getPage(this.props.pages, _.get(this.props, 'page.category', null).stackbit_url_path);
                                return (
                                    <h2 className="product__category">
                                        <Link href={withPrefix(_.get(category_page, 'stackbit_url_path', null))}> {_.get(category_page, 'title', null)} </Link>
                                    </h2>
                                );
                            })())}
                            <div className="product__price">
                                ${_.get(this.props, 'page.price', null)}
                            </div>
                            <article className="product__description">
                                {_.get(this.props, 'page.description', null)}
                            </article>
                            <BuyButton {...this.props} product_page={this.props.page} />
                        </div>
                    </section>
                    {_.get(this.props, 'page.category', null) && ((() => {
                        let product_pages = _.filter(_.orderBy(getPages(this.props.pages, '/products'), 'order'), item => _.get(item, 'id') != _.get(this.props, 'page.id', null));
                        let category_page = getPage(this.props.pages, _.get(this.props, 'page.category', null).stackbit_url_path);
                        return (
                            <section className="content__row">
                                <h2 className="content__row-title">Related</h2>
                                <ProductGrid {...this.props} product_pages={product_pages} category_url={_.get(category_page, 'stackbit_url_path', null)} cssClass={'store__product-grid'} site={this.props} />
                            </section>
                        );
                    })())}
